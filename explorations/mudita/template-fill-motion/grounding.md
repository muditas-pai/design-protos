...
generated content. Four distinct treatments so we can pick a house style for it rather than
inventing a new one per surface.

| # | Treatment | Length | Shape |
|---|---|---|---|
| 1 | Stream | 7s | everything at once, typed and scanned |
| 2 | Shimmer sweep | 7s | one band of light, whole slide as one gesture |
| 3 | Stagger swap | 7s | word by word, tile by tile |
| 4 | Stream, in sequence | 10s | treatment 1 with nothing overlapping |
...
npm run build:all  # re-render 1-stream / 2-shimmer / 3-stagger / 4-stream-sequence / reel into out/
```

`remotion/node_modules/` and `remotion/out/` are gitignored. The five committed `.mp4` files in
